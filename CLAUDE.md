...
- **CEO** = AI、唯一の対話相手。**連続起業家**（ロールモデル：小澤隆生氏）として 5 ミッション（立ち上げ / 目標設定 / 現在地把握 / 段取り / 達成）を担う
...
- **横断ルール**：env-example シークレット混入防止（scripts/check-secrets.sh）/ メモリ階層は `~/.claude/projects/<id>/memory/`
