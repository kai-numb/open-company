...
status: active
purpose: |
  open-company 配下で新規事業を立ち上げるとき、CEO が機械的に通すチェックリスト。
...
## Phase 2：事業フォルダ作成（10 分）

```bash
# open-company 配下で実行
src=open-company/skel/new-business-template
biz_dir=open-company/biz/[事業名]

# 1. 事業ディレクトリ作成（管理層 + コード/アセット用の親）
mkdir -p $biz_dir/{inbox,todos,notes}

# 2. CLAUDE.md（事業用）を skel から複製、事業名を書き換え
cp open-company/biz/drone-vibe-game/CLAUDE.md $biz_dir/CLAUDE.md
# → 事業名・概要・コードベース所在・差別化ポイントを書き換え

# 3. 当日の意思決定ノートを起こす
touch $biz_dir/notes/$(date +%Y-%m-%d)-decisions.md
```

- [ ] `open-company/biz/[事業名]/` ディレクトリ作成
- [ ] `CLAUDE.md`（事業概要・コードベース所在・ルール）作成
- [ ] `notes/YYYY-MM-DD-decisions.md` で立ち上げ決定を記録
- [ ] `open-company/.company/CLAUDE.md` の「事業一覧」に追記
...
```bash
# 実コード側の場所（biz/[事業名]/code/、submodule として親リポに紐付け）に skel を複製
src=/Users/kaihasunuma/Desktop/Claude-test/open-company/skel/new-business-template
dst=/Users/kaihasunuma/Desktop/Claude-test/open-company/biz/[事業名]/code
...
cp open-company/.company/ceo/notes/templates/scripts/check-secrets.sh $dst/scripts/
cp open-company/.company/ceo/notes/templates/scripts/install-hooks.sh $dst/scripts/
...
- [ ] git init して initial commit
  ...
  git init
  git add -A
  git commit -m "chore: initial scaffold from open-company skel"
  ```
