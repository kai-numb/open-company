...
biz_mgmt=open-company/.company/biz/[事業名]

# 1. 管理層ディレクトリ作成
mkdir -p $biz_mgmt/{inbox,todos,notes}

# 2. CLAUDE.md（管理層用）を skel から複製、事業名を書き換え
cp open-company/.company/biz/drone-vibe-game/CLAUDE.md $biz_mgmt/CLAUDE.md
...
- [ ] `open-company/.company/biz/[事業名]/` ディレクトリ作成
...
```bash
# 実コード側の場所（businesses/ または open-company/biz-code/）に skel を複製
...
# (submodule 化後は: dst=open-company/biz-code/[事業名])
